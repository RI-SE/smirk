...
The pedestrian recognition component relies on [transfer learning](https://www.tensorflow.org/tutorials/images/transfer_learning). The ML model is pre-trained on the dataset for object detection that is part of the publicly available [KITTI Vision Benchmark Suite](http://www.cvlibs.net/datasets/kitti/). The dataset contains 7,481 training images annotated with bounding boxes whose quality has been assured through numerous research studies, qualifying as an artifact that has been [proven in use](https://www.61508.org/images/downloads/Proven_in_use_policy_v7.pdf). Pretraining on the KITTI dataset provides features from real-world imagery, from basic shapes to more complex features, that diversifies the feature learning to ensure a more robust model. Subsequently, the SMIRK pedestrian recognition component is fine-tuned for the task of pedestrian detection in the specific ODD through transfer learning. The [data requirements](https://github.com/RI-SE/smirk/blob/main/docs/Data%20Management%20Specification.md#2-data-requirements-l-) address the dataset used for the fine-tuning.
