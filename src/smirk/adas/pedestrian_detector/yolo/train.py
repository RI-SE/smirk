...
#
"""
Custom yolov5 train script adapted from ultralytics/yolov5 train.py
"""
...
from smirk.adas.pedestrian_detector.yolo.metrics import (
    fitness,  # Use SMIRK fitness function
)
...
def run_training(
    hyp, opt, device, callbacks, names=("pedestrian")
    ...
    data_dict = dict(train=train_path, val=val_path, nc=nc, names=names)
    ...
    assert (
        mlc < nc
    ), f"Label class {mlc} exceeds nc={nc}. Possible class labels are 0-{nc - 1}"
...
def parse_opt(known=False):
    ...
    parser.add_argument("--train-path", type=str, help="path to training data dir")
    parser.add_argument("--val-path", type=str, help="path to val data dir")
    parser.add_argument(
        "--hyp",
        type=str,
...
def main(opt, callbacks):
    ...
    # Train
    run_training(opt.hyp, opt, device, callbacks)
